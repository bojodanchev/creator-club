  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-slate-500 text-sm">{plan.description || getDefaultDescription(plan.tier)}</p>
      </div>
      ...
      <div className="text-center mb-6">
        <div className="flex items-baseline justify-center gap-1">
          <span className="text-4xl font-bold text-slate-900">
            {plan.price_monthly_cents === 0 ? 'Free' : `${plan.price_monthly_cents / 100}`}
          </span>
      ...
      {displayInfo.breakEvenRevenue && (
        <div className="text-center mb-4 px-3 py-2 bg-indigo-50 rounded-lg">
          <p className="text-xs text-indigo-700">
            Best value when earning {displayInfo.breakEvenRevenue}+
          </p>
        </div>
      )}
...
/**
 * Get default description for a plan tier
 */
function getDefaultDescription(tier: PlanTier): string {
  switch (tier) {
    case 'starter':
