  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-lg font-bold text-slate-900">Tasks & Reminders</h2>
        <button
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
        {(['all', 'todo', 'in_progress', 'done'] as const).map((status) => (
          ...
          >
            {status === 'all' ? 'All' : getTaskStatusLabel(status)} ({taskCounts[status]})
          </button>
        ))}
      </div>
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <button
                  ...
                  title={`Click to change status to ${getTaskStatusLabel(getNextStatus(task.status))}`}
                >
          ...
          <div className="text-center py-12 text-slate-400">
            ...
            <p className="text-xs mt-1">
              {filter === 'all'
                ...
                : `No ${getTaskStatusLabel(filter as TaskStatus).toLowerCase()} tasks`}
            </p>
          </div>
        )}
      </div>
