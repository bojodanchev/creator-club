  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h3 className="font-semibold text-gray-900 text-lg">{packageName}</h3>
            ...
            {statusOrder.map((status, index) => (
              <div key={status} className="flex items-center">
                <div className="flex flex-col items-center">
                  ...
                  <span className={`text-xs mt-1 ${index <= currentStep ? 'text-purple-600' : 'text-gray-400'}`}>
      ...
      <div className="grid grid-cols-2 gap-4 text-sm mb-4">
        <div>
          ...
          <span className="ml-2 text-gray-900">{application.business_name || '-'}</span>
        </div>
        <div>
          <span className="text-gray-500">Revenue:</span>
          <span className="ml-2 text-gray-900">{application.current_revenue || '-'}</span>
        </div>
      </div>
