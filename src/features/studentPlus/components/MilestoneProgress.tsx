  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="font-semibold text-lg text-gray-900">Milestone Progress</h3>
        {nextMilestone && (
          <span className="text-sm text-gray-500">
            {nextMilestone.months_required - consecutiveMonths} months to {nextMilestone.name}
          </span>
        )}
      </div>
        ...
        <div className="flex justify-between mt-4">
          {milestonesWithProgress.map((milestone) => (
            ...
            >
              ...
              <span className="text-xs opacity-75">{milestone.months_required}mo</span>
              {milestone.achieved && (
                <span className="text-xs text-green-600 font-medium mt-1">Achieved!</span>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="flex items-center justify-between p-4 bg-purple-50 rounded-lg">
        <div>
          <div className="text-sm text-purple-700 font-medium">Current Progress</div>
        ...
        {nextMilestone && (
          <div className="text-right">
            <div className="text-sm text-purple-700 font-medium">Next Reward</div>
            <div className="text-lg font-bold text-purple-900">
              +{nextMilestone.bonus_points} points
            </div>
        ...
        {!nextMilestone && consecutiveMonths >= 12 && (
          <div className="text-right">
            <div className="text-sm text-purple-700 font-medium">Status</div>
            <div className="text-lg font-bold text-purple-900">Diamond Member</div>
            <div className="text-xs text-purple-600">All milestones achieved!</div>
          </div>
        )}
      </div>
    </div>
