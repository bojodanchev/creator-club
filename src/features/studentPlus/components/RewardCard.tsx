...
export function RewardCard({ reward, userPoints, onRedeem, isRedeeming }: RewardCardProps) {
  ...
  const getRewardDetails = (): string => {
    ...
    switch (reward.reward_type) {
      case 'voucher':
        return `${config.discount_percent}% off - Valid for ${config.valid_days} days`;
      case 'priority_support':
        return `${config.duration_days} days of priority support`;
      case 'fee_discount':
        return `${config.discount_percent}% fee reduction for ${config.duration_days} days`;
      case 'template_pack':
        return 'Downloadable templates package';
      default:
        return '';
    }
  };
