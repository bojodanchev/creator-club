    ...
    <div className="bg-white border border-gray-200 rounded-xl p-6">
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <h2 className="text-xl font-bold text-gray-900">Student Plus</h2>
            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColor[subscription.status]}`}>
              {statusLabel[subscription.status]}
            </span>
          </div>
          <p className="text-gray-600 text-sm">
            {formatPrice(STUDENT_PLUS_CONFIG.product.amount)}/month
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-gray-100">
        <div>
          ...
          <div className="text-xs text-gray-500">Months Active</div>
        </div>
        <div>
      ...
      {subscription.cancel_at_period_end && (
        <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          <p className="text-sm text-yellow-800">
            ...
            {formatDate(subscription.current_period_end)}. You'll continue to have access until then.
          </p>
        </div>
      )}
    </div>
