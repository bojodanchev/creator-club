...
import { getStripeClient, STRIPE_CONFIG, PlanTier } from '../_shared/stripe.ts';
...
// Creates a one-time payment session for the EUR 2.90 activation fee
// ============================================================================
...
async function handleActivationCheckout(
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: [
      {
        price: STRIPE_CONFIG.activation.priceId,
        quantity: 1,
      },
    ],
...
async function handleSubscriptionCheckout(
  ...
  if (!plan?.stripe_price_id) {
    return errorResponse('Plan not found or has no Stripe price configured');
  }
